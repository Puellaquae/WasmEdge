...
wasm::Val conv_val_from_wasmedge(const WasmEdge::ValVariant &v,
                                 WasmEdge::ValType vt) {
...
WasmEdge::ValVariant conv_val_to_wasmedge(const wasm::Val &v) {
  ...
  case wasm::ValKind::F32:
    return static_cast<float>(v.f32());
  case wasm::ValKind::F64:
...
wasm::Val wasm::Global::get() const {
  ...
  return conv_val_from_wasmedge(inst.getValue(),
...
void wasm::Global::set(const wasm::Val &val) {
  ...
  static_cast<wasm_global_t *>(this)->keepval = val.copy();
  static_cast<wasm_global_t *>(this)->inst.getValue() =
      conv_val_to_wasmedge(val);
}
...
wasm::own<wasm::Ref> wasm::Table::get(wasm::Table::size_t index) const {
  ...
  if (auto res = inst.getRefAddr(static_cast<uint32_t>(index));
    ...
    if (inst.getTableType().getRefType() == WasmEdge::RefType::FuncRef) {
      ...
      WasmEdge::retrieveExternRef<wasm_ref_t *>(*res)->copy_ref();
    }
  }
  return nullptr;
}
